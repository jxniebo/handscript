...
void adjust_nodes(Node** nodes, int min, int min_step, int size) {
  ...
  new_node->lsize = new_node->lchild->lsize + new_node->lchild->rsize + 1;
  new_node->rsize = new_node->rchild->lsize + new_node->rchild->rsize + 1;
...
Code* decode_character(Node* head, Code** code, char character) {
  while(head && (head->lchild || head->rchild)) {
    if(!head->lchild && !head->rchild) {
      if(*head->contains == character) {
        return *code;
      }
    } else if(head->lchild) {
      //      if(contains_char(head->lchild, character)) {
      //head->lchild;
      }
